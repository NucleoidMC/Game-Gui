...
public class JoinOpenedGameEntry extends GuiEntry {
    private final GameConfigMetadata config;
    private ManagedGameSpace gameSpace;

    protected JoinOpenedGameEntry(GameConfigMetadata config) {
        super(config.getIconOr(new ItemStack(Blocks.BARRIER)));
        this.config = config;
    }

    public JoinOpenedGameEntry withGameSpace(ManagedGameSpace gameSpace) {
